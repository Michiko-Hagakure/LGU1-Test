...
// --- ONE-FILE FIX FOR 404 AND DASHBOARD ---

// STEP A: The Fixer. Visit this URL once: 
// https://facilities.local-government-unit-1-ph.com/clear-cache
Route::get('/clear-cache', function() {
    \Artisan::call('route:clear');
    \Artisan::call('cache:clear');
    return "<h1>404 Fix Successful!</h1><p>System refreshed. <a href='/view-funds'>Go to Dashboard</a></p>";
});

// STEP B: The Dashboard
Route::get('/view-funds', function () {
    $data = \App\Models\FundRequest::orderBy('id', 'desc')->get();
    
    echo '<script src="https://cdn.tailwindcss.com"></script>';
    echo '<div class="p-8 bg-gray-100 min-h-screen font-sans">';
    echo '<div class="max-w-7xl mx-auto bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-200">';
    
    // Header
    echo '<div class="bg-slate-900 p-8 text-white flex justify-between items-center border-b-4 border-blue-600">
            <div>
                <h1 class="text-3xl font-black tracking-tight uppercase">Facility & Fund Requests</h1>
                <p class="text-blue-400 text-xs font-bold uppercase tracking-widest mt-1">LGU Centralized Management</p>
            </div>
            <div class="bg-blue-600/20 px-6 py-3 rounded-xl border border-blue-500/30">
                <span class="text-blue-300 text-[10px] font-bold uppercase block mb-1">Source Office</span>
                <span class="font-black text-white">ENERGY DEPARTMENT</span>
            </div>
          </div>';

    echo '<table class="w-full text-left">';
    echo '<thead class="bg-gray-50 border-b border-gray-200 text-[11px] font-black text-gray-400 uppercase tracking-widest">
            <tr>
                <th class="px-6 py-5">Ref ID</th>
                <th class="px-6 py-5">Requestor</th>
                <th class="px-6 py-5">Project/Purpose</th>
                <th class="px-6 py-5">Required Logistics</th>
                <th class="px-6 py-5 text-right">Fund Amount</th>
                <th class="px-6 py-5 text-center">Action</th>
            </tr>
          </thead>';
    echo '<tbody class="divide-y divide-gray-100 bg-white">';

    foreach ($data as $row) {
        $statusColor = ($row->status == 'Approved') ? "bg-emerald-500" : "bg-amber-500";

        echo "<tr class='hover:bg-blue-50/20 transition'>";
        echo "<td class='px-6 py-6 font-bold text-blue-600 text-sm'>#ENRG-0$row->id</td>";
        
        echo "<td class='px-6 py-6'>
                <div class='text-sm font-bold text-gray-900'>$row->requester_name</div>
                <div class='text-[10px] text-gray-400 uppercase'>User ID: $row->user_id</div>
              </td>";

        echo "<td class='px-6 py-6 text-sm text-gray-600 font-medium'>$row->purpose</td>";

        // Display Logistics (Speakers, Venues, etc)
        echo "<td class='px-6 py-6'>
                <div class='bg-gray-50 p-2 rounded-lg border border-gray-100 text-[11px] text-gray-500 italic'>
                    " . ($row->logistics ? str_replace(', ', '<br>• ', $row->logistics) : 'No logistics items') . "
                </div>
              </td>";

        echo "<td class='px-6 py-6 text-right font-black text-gray-900 text-sm'>₱" . number_format($row->amount, 2) . "</td>";
        
        echo "<td class='px-6 py-6 text-center'>";
        if($row->status == 'pending') {
            echo "<div class='flex gap-2 justify-center'>
                    <a href='/approve-fund/$row->id/Approved' class='bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase transition'>Approve</a>
                    <a href='/approve-fund/$row->id/Rejected' class='bg-rose-600 hover:bg-rose-700 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase transition'>Reject</a>
                  </div>";
        } else {
            echo "<span class='inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase $statusColor text-white'>
    ...
    // Inside your /view-funds route, add this <script> at the bottom
echo "
<script>
    ...
    setInterval(refreshData, 5000); // Update government view every 5 seconds
</script>";
});