...
@section('page-title', 'Edit Your Review')
@section('page-subtitle', 'Update your feedback for this facility')

@section('page-content')
<div class="max-w-4xl mx-auto space-y-6">
    <!-- Back Button -->
    <div>
        <a href="{{ URL::signedRoute('citizen.reviews.index') }}" 
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden border-2 border-lgu-green">
        ...
        <div class="p-6">
            <div class="flex gap-6">
                ...
                <div class="flex-1">
                    <h3 class="text-2xl font-bold text-lgu-headline mb-2">{{ $review->facility_name }}</h3>
                    <p class="text-sm text-gray-600 mb-3">{{ $review->facility_address }}</p>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <p class="text-xs text-gray-500 font-medium">Event Date</p>
                            <p class="text-sm font-semibold text-gray-900">
                                {{ \Carbon\Carbon::parse($review->start_time)->format('F d, Y') }}
                            </p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 font-medium">Event Time</p>
                            <p class="text-sm font-semibold text-gray-900">
                                {{ \Carbon\Carbon::parse($review->start_time)->format('g:i A') }} - 
                                {{ \Carbon\Carbon::parse($review->end_time)->format('g:i A') }}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    <!-- Review Form -->
    <form action="{{ URL::signedRoute('citizen.reviews.update', $review->id) }}" method="POST" class="bg-white shadow rounded-lg overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="flex items-center gap-2">
                    <div id="star-rating" class="flex gap-1">
                        @for ($i = 1; $i <= 5; $i++)
                            <button type="button" 
                                    ...
                                    data-rating="{{ $i }}">
                                <svg class="w-12 h-12 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-gray-300' }} hover:text-yellow-400 transition-colors" 
                                     fill="currentColor" 
                                     viewBox="0 0 20 20">
                    ...
                    <span id="rating-text" class="text-lg font-semibold text-lgu-highlight ml-2">
                        ...
                        @endphp
                        {{ $labels[$review->rating] ?? 'No rating selected' }}
                    </span>
                </div>
            ...
            <div>
                ...
                <textarea 
                    ...
                    placeholder="Tell us about your experience with this facility. What did you like? What could be improved?">{{ old('review', $review->review) }}</textarea>
